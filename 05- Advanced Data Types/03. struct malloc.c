...
structures as well as an integer, indicating how many points can be stored in the array. 
...
then allocate the appropriate amount of memory, initialize the array with the function initializePoly, 
...
int main(void) {
    ...
    scanf("%d", &N);
    struct point *polygon = (struct point*)malloc(N* sizeof(struct point));
